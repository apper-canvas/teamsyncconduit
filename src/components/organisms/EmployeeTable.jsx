...
<td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex flex-wrap gap-1">
                      {employee.name17_c && employee.name17_c.split(',').filter(v => v.trim()).map((value, index) => (
                        <span key={index} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                          {value.trim()}
                        </span>
                      ))}
                      {(!employee.name17_c || employee.name17_c.split(',').filter(v => v.trim()).length === 0) && (
                        <span className="text-gray-500">-</span>
                      )}
                    </div>
</td>
<td className="py-3 px-4">
                    <div className="flex flex-wrap gap-1">
                      {employee.name18_c && employee.name18_c.split(',').filter(v => v.trim()).map((value, index) => (
                      ...
                      {(!employee.name18_c || employee.name18_c.split(',').filter(v => v.trim()).length === 0) && (
                        <span className="text-gray-500">-</span>
                      )}
                    </div>
                  </td>
